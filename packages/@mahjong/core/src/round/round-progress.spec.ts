...
describe('RoundProgress', () => {
  describe('compareTo', () => {
    it('同じ値を持つ Point を与えられた場合、0 を返すこと', () => {
      const sut = new RoundProgress(RoundWind.East, new RoundIndex(1, 4));
      const target = new RoundProgress(RoundWind.East, new RoundIndex(1, 4));

      expect(sut.compareTo(target)).toBe(0);
    });

    it('自身より前の RoundProgress を与えられた場合、1 以上を返すこと', () => {
      const sut = new RoundProgress(RoundWind.East, new RoundIndex(2, 4));
      const target = new RoundProgress(RoundWind.East, new RoundIndex(1, 4));
    ...
    it('自身より後の RoundProgress を与えられた場合、-1 以下を返すこと', () => {
      const sut = new RoundProgress(RoundWind.East, new RoundIndex(1, 4));
      const target = new RoundProgress(RoundWind.East, new RoundIndex(2, 4));
  ...
  describe('equals', () => {
    ...
    it('異なる値を与えられた場合、false を返すこと', () => {
      ...
      expect(
        sut.equals(new RoundProgress(RoundWind.West, new RoundIndex(1, 4))),
      ).toBe(false);
      expect(
        sut.equals(new RoundProgress(RoundWind.East, new RoundIndex(2, 4))),
      ).toBe(false);
    });
  });
});
